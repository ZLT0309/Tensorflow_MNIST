...
def load_data_wrapper():
    train_d,validation_d,test_d = load_data()
    training_inputs=[np.reshape(x,(784,1)) for x in train_d[0]]
    training_results = [vectorized_result(y) for y in train_d[1]]
    training_data = list(zip(training_inputs,training_results))
    validation_inputs = [np.reshape(x,(784,1)) for x in test_d[0]]
    validation_data = list(zip(validation_inputs,validation_d[1]))
...
model = Model([784,80,60,30,10])
train_D,validation_D,test_D = load_data_wrapper()

model.SGD(train_D,10,30,3,test_D)